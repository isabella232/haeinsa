...
/**
 * https://issues.vcnc.co.kr/browse/HAEINSA-67 When Haeinsa recovers rows, we should make stable
 * rows into new committimestamp's stable rows. This method prevents making dangling row locks of
  ...
  public void testRecover() throws Exception {
...

    {
      ...
      TRowLock secondaryRowLock =
          new TRowLock(HaeinsaConstants.ROW_LOCK_VERSION, TRowLockState.STABLE, 1380504156137L);
      TRowLock primaryRowLock =
          new TRowLock(HaeinsaConstants.ROW_LOCK_VERSION, TRowLockState.PREWRITTEN, 1380504157100L)
      ...
      TRowLock stablePrimaryRowLock =
          TRowLocks.deserialize(
              primaryResult.getValue(
                  HaeinsaConstants.LOCK_FAMILY, HaeinsaConstants.LOCK_QUALIFIER));
      Assert.assertEquals(stablePrimaryRowLock.getState(), TRowLockState.STABLE);
      Assert.assertEquals(
          stablePrimaryRowLock.getCommitTimestamp(), primaryRowLock.getCommitTimestamp());
      ...
      TRowLock stableSecondaryRowLock =
          TRowLocks.deserialize(
              secondaryResult.getValue(
                  HaeinsaConstants.LOCK_FAMILY, HaeinsaConstants.LOCK_QUALIFIER));
      Assert.assertEquals(stableSecondaryRowLock.getState(), TRowLockState.STABLE);
      Assert.assertEquals(
          stableSecondaryRowLock.getCommitTimestamp(), primaryRowLock.getCommitTimestamp());
    }
