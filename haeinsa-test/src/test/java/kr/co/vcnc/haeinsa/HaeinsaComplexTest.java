...
/**
 * Complex multi-thread unit test for Haeinsa. It contains simple-increment test, concurrent random
 * increment test, and serializability test.
 */
public class HaeinsaComplexTest extends HaeinsaTestBase {

  /**
   * Test which executes multiple transactions which increment specific value by single thread and
   * check result.
   */
  @Test
  ...
  /**
   * Test which execute multiple transactions by multiple threads concurrently which increase value
   * of single row randomly. Check result after transactions with value in local variable.
   */
  @Test
  public void testConcurrentRandomIncrement() throws Exception {
    ...
    Runnable singleIncrementJob =
        new Runnable() {
          ...
          public void run() {
            ...
            System.out.println(
                String.format(
                    "iteration : %d on Thread : %s", iteration, Thread.currentThread().getName()));
    ...
    ExecutorService service =
        Executors.newFixedThreadPool(
            numberOfJob, new ThreadFactoryBuilder().setNameFormat("IncrementJobThread-%d").build());
    ...
    System.out.println(
        "Conflict rate : "
            + failCount.get() / ((double) failCount.get() + (double) successCount.get()) * 100.0);
  ...
  /**
   * Test serializability. Start with writing random value in row1 and row2. Execute transaction by
   * multiple threads concurrently. Each transaction will get value from DB and write new value on
   * the row based on previous value. Pseudo code is as follow
   *
   * <pre>
   ...
   * </pre>
   *
   * If transaction successes, it will acquire local memory lock and atomically change two
   * atomicInteger in local. After multiple times of concurrent transaction, if data in memory and
   * DB is same then we can think this schedule is serializable.
   */
  // @Test
    ...
    Runnable serialJob =
        new Runnable() {
          ...
          public void run() {
            ...
            while (iteration < maxIter) {
              try {
                ...
                long oldValue1 =
                    Bytes.toLong(
                        testTable
                            .get(tx, new HaeinsaGet(row1).addColumn(CF, CQ1))
                            .getValue(CF, CQ1));
                long oldValue2 =
                    Bytes.toLong(
                        testTable
                            .get(tx, new HaeinsaGet(row2).addColumn(CF, CQ2))
                            .getValue(CF, CQ2));
            ...
            System.out.println(
                String.format(
                    "iteration : %d on Thread : Ts", iteration, Thread.currentThread().getName()));
            countDownLatch.countDown();
          }
        };
