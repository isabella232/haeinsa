 ...
 * HaeinsaMutation can handle multiple Puts or Deletes of single row, but single HaeinsaMutation
 * instance can not represent both at same time. {@link HaeinsaPut} and {@link HaeinsaDelete} are
 * implementations of this class.
 *
 * <p>This class assists HaeinsaTable to project history of Puts/Deletes inside transaction to
 * Get/Scan operations which is executed after. These late Get/Scan operations can see the mutated
 * view of specific row even before those mutations are committed to HBase.
 *
 * <p>HaeinsaMutation provides {@link HaeinsaKeyValueScanner} interface by {@link #getScanner(long)}
 * method.
 */
public abstract class HaeinsaMutation extends HaeinsaOperation {
   ...
   *
   * <p>Iterator provided by MutationScanner uses {@link HaeinsaKeyValue#COMPARATOR} to sort
   * HaeinsaKeyValue in {@link HaeinsaMutation#familyMap}.
   *
   * <p>All HaeinsaKeyValue provided by single MutationScanner have same sequenceID.
   */
  private class MutationScanner implements HaeinsaKeyValueScanner {
    ...
    /**
     * Iterator provided by MutationScanner access to sorted list of HaeinsaKeyValue by {@link
     * HaeinsaKeyValue#COMPARATOR}.
     *
     * <p>The part of generating iterator in this constructor based on the assumption that values()
     * function of TreeMap return sorted collection of values. Otherwise, the way to generate
     * iterator should be changed.
     *
     * @param sequenceID sequence id represent which Scanner is newer one. Lower id is newer one.
     */
