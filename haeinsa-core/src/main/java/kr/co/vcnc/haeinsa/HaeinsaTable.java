...
/**
 * Implementation of {@link HaeinsaTableIface}. It works with {@link HaeinsaTransaction} to provide
 * transaction on HBase.
 */
public class HaeinsaTable implements HaeinsaTableIfaceInternal {
  ...
  /**
   * Get data from HBase without transaction. {@link HaeinsaTransaction#commit()} to check or mutate
   * lock column of the row scanned by this method. This method can be used when read performance is
   * important or strict consistency of the result is not matter.
   */
  ...
  public HaeinsaResult get(@Nullable HaeinsaTransaction tx, HaeinsaGet get) throws IOException {
    ...
    try (ClientScanner scanner =
        new ClientScanner(tx, scanners, get.getFamilyMap(), lockInclusive, false)) {
  ...
  @Override
  public HaeinsaResultScanner getScanner(
      @Nullable HaeinsaTransaction tx, byte[] family, byte[] qualifier) throws IOException {
    Preconditions.checkNotNull(family);
  ...
  /**
   * Haeinsa implementation of {@link Scan}. Scan range of row inside defined by {@link HaeinsaScan}
   * in the context of transaction(tx). Return ClientScanner which related to {@link HaeinsaTable}
   * and {@link HaeinsaTransaction}.
   *
   * <p>Return {@link SimpleClientScanner} which do not support transaction if tx is null. User can
   * use this feature if specific scan operation does not require strong consistency or high
   * performance is crucial because scan too wide range of rows.
   */
  @Override
  ...
  /**
   * Scan data from HBase without transaction. {@link HaeinsaTransaction#commit()} to check or
   * mutate lock column of the row scanned by this method. This method can be used when read
   * performance is important or strict consistency of the result is not matter.
   *
  ...
  /**
   * Haeinsa implementation of {@link ColumnRangeFilter}. Scan range of column inside single row
   * defined by {@link HaeinsaIntraScan} in the context of transaction(tx). Return {@link
   * ClientScanner} which related to {@link HaeinsaTable} and {@link HaeinsaTransaction}.
   *
   * <p>Return {@link SimpleClientScanner} which do not support transaction if tx is null. User can
   * use this feature if specific intra-scan operation does not require strong consistency.
   *
   * <p>Haeinsa does not support column-range scan over multiple row in this version.
   */
  @Override
  public HaeinsaResultScanner getScanner(
      @Nullable HaeinsaTransaction tx, HaeinsaIntraScan intraScan) throws IOException {
  ...
  /**
   * Scan data inside single row (intraScan) without transaction. {@link
   * HaeinsaTransaction#commit()} to check or mutate lock column of the row scanned by this method.
   * This method can be used when read performance is important or strict consistency of the result
   * is not matter.
   *
   * @throws IOException IOException from HBase.
   */
   ...
   *
   * <p>Get {@link TRowLock} of the row from HBase if rowState does not contains it. If lock is not
   * in stable state, try to recover it first by {@link HaeinsaTransaction#recover(boolean)}.
   *
   * <p>By calling this method proper time, {@link HaeinsaRowTransaction} inside {@link
   * HaeinsaTransaction} can have {@link TRowLock} of the row when this method was called first time
   * in the context of the transaction.
   *
   * @throws IOException ConflictException, HBase IOException
   */
  private HaeinsaRowTransaction checkOrRecoverLock(
      HaeinsaTransaction tx,
      byte[] row,
      HaeinsaTableTransaction tableState,
  ...
  /**
   * Check whether specific row need recover by checking {@link TRowLock}. Return true only if
   * rowLock is NOT in {@link TRowLockState#STABLE} state and lock is expired. Return false if
   * rowLock is in {@link TRowLockState#STABLE} state. Throw {@link ConflictException} if rowLock is
   * not in stable state and not expired yet.
   *
   * @return true - when lock is established but expired. / false - when there is no lock ( {@link
   *     TRowLockState#STABLE} )
   * @throws IOException {@link NotExpiredYetException} if lock is established and not expired.
   */
  ...
  /**
   * Call {@link HaeinsaTransaction#recover(boolean)}. Abort or recover when there is failed
   * transaction on the row, throw {@link ConflictException} when there is ongoing transaction.
   *
   * @throws IOException ConflictException, HBase IOException
   */
  ...
  public void delete(HaeinsaTransaction tx, HaeinsaDelete delete) throws IOException {
    ...
    // Can't delete entire row in Haeinsa because of lock column. Please specify column families
    // when needed.
  ...
  public void commitSingleRowPutOnly(HaeinsaRowTransaction rowState, byte[] row)
    ...
    TRowLock newRowLock =
        new TRowLock(ROW_LOCK_VERSION, TRowLockState.STABLE, tx.getCommitTimestamp());
    put.addColumn(
        LOCK_FAMILY, LOCK_QUALIFIER, tx.getCommitTimestamp(), TRowLocks.serialize(newRowLock));
  ...
  /**
   * Read {@link TRowLock} from HBase and compare that lock with prevRowLock. If TRowLock is
   * changed, it means transaction is failed, so throw {@link ConflictException}.
   *
   * @throws IOException ConflictException, HBase IOException.
  ...
  public void checkSingleRowLock(HaeinsaRowTransaction rowState, byte[] row) throws IOException {
    ...
    if (!rowState.getCurrent().equals(currentRowLock)) {
      HaeinsaTransaction tx = rowState.getTableTransaction().getTransaction();
      HaeinsaTransaction currentTx =
          tx.getManager().getTransaction(tx.getPrimary().getTableName(), tx.getPrimary().getRow());
      if (currentTx != null) {
      ...
      throw new ConflictException(
          "this row is modified, checkSingleRow failed: " + new String(row));
    }
  }
  ...
  public void prewrite(HaeinsaRowTransaction rowState, byte[] row, boolean isPrimary)
    ...
    if (rowState.getMutations().size() > 0) {
      if (rowState.getMutations().get(0) instanceof HaeinsaPut) {
        ...
        for (HaeinsaKeyValue kv : Iterables.concat(haeinsaPut.getFamilyMap().values())) {
          put.addColumn(
              kv.getFamily(), kv.getQualifier(), tx.getPrewriteTimestamp(), kv.getValue());
    ...
    TRowLock newRowLock =
        new TRowLock(ROW_LOCK_VERSION, TRowLockState.PREWRITTEN, tx.getCommitTimestamp())
            .setCurrentTimestamp(tx.getPrewriteTimestamp());
    if (isPrimary) {
      // for primary row
      for (Entry<TRowKey, HaeinsaRowTransaction> rowStateEntry :
          tx.getMutationRowStates().entrySet()) {
    ...
    put.addColumn(
        LOCK_FAMILY, LOCK_QUALIFIER, tx.getPrewriteTimestamp(), TRowLocks.serialize(newRowLock));
        ...
        case PUT:
          {
            ...
            put.addColumn(
                LOCK_FAMILY,
                LOCK_QUALIFIER,
                newRowLock.getCurrentTimestamp(),
        ...
        case REMOVE:
          {
            ...
            if (mutation.getRemove().getRemoveCellsSize() > 0) {
              for (TCellKey removeCell : mutation.getRemove().getRemoveCells()) {
                delete.addColumns(
                    removeCell.getFamily(), removeCell.getQualifier(), mutationTimestamp);
              }
            }
            if (!table.checkAndDelete(
                row, LOCK_FAMILY, LOCK_QUALIFIER, currentRowLockBytes, delete)) {
              // Consider as conflict because another transaction might acquire lock of this row.
              throw new ConflictException("can't acquire row's lock");
            }
            break;
  ...
  public void deletePrewritten(HaeinsaRowTransaction rowTxState, byte[] row) throws IOException {
    ...
    byte[] currentRowLockBytes = TRowLocks.serialize(rowTxState.getCurrent());
    long prewriteTimestamp =
        rowTxState.getCurrent().isSetPrewriteTimestamp()
            ? rowTxState.getCurrent().getPrewriteTimestamp()
            : rowTxState.getCurrent().getCurrentTimestamp();
    Delete delete = new Delete(row);
  ...
  private class SimpleClientScanner implements HaeinsaResultScanner {
    ...
    public Iterator<HaeinsaResult> iterator() {
      return Iterators.transform(
          scanner.iterator(),
          new Function<Result, HaeinsaResult>() {
            @Override
  ...
  /**
   * Contains scanners for single {@link HaeinsaTable} to help project puts/deletes to gets/scans in
   * same transaction.
   *
   * <p>This projection of mutations is crucial to proper execution of transaction. For instance,
   * consider single transaction such as T = {W1(X), R2(X), R3(Y), W4(X)}. Haeinsa does not write
   * mutations on HBase until commit() is called. Therefore if R(X) get data only from HBase, R2(X)
   * on transaction T cannot read data written by W1(X) and this is not expected behavior to
   * programmers. Haeinsa resolves this problem by projecting buffered mutations in client to
   * get/scan operations executed in same transaction.
   */
  private class ClientScanner implements HaeinsaResultScanner {
    ...
    /**
     * true if TRowLock inside scanners, false if TRowLock is already included inside
     * tableState.getRowStates().get(row)
     */
    ...
    /**
     * @param lockInclusive - whether scanners contains {@link TRowLock} inside. If not, should
     *     bring from {@link HaeinsaRowTransaction} or get from HBase directly.
     */
    public ClientScanner(
        HaeinsaTransaction tx,
        Iterable<HaeinsaKeyValueScanner> scanners,
        Map<byte[], NavigableSet<byte[]>> familyMap,
        boolean lockInclusive,
        boolean reversed) {
    ...
    public ClientScanner(
      ...
      this.columnTracker =
          new HaeinsaColumnTracker(
              familyMap,
              intraScan.getMinColumn(),
              intraScan.isMinColumnInclusive(),
              intraScan.getMaxColumn(),
              intraScan.isMaxColumnInclusive());
      ...
      this.scanners =
          Sets.newTreeSet(
              !reversed
                  ? HaeinsaKeyValueScanner.COMPARATOR
                  : HaeinsaKeyValueScanner.REVERSE_COMPARATOR);
    }

    /**
     * Method to generate {@link #scanners} from {@link #scannerList}. Only can be called one time
     * for every ClientScanner.
     *
     * <p>The reason why there are different variables for {@link #scannerList} and {@link
     * #scanners} is that the way {@link #nextScanner(HaeinsaKeyValueScanner)} implemented is
     * removing {@link HaeinsaKeyValueScanner} one by one form scanners. {@link #close()} method
     * needs to close every {@link ClientScanner} when called, so some other variable should
     * preserve every scanner when ClientScanner created.
     */
    ...
    /**
     * Return {@link TRowLock} for specific row from {@link #scanners}. Return null if there is no
     * proper {@link TRowLock}.
     *
     * <p>If one of these {@link #scanners} has row key which is smaller than given row, that {@link
     * HaeinsaKeyValueScanner} is not peeked by this method and return null. So proper operation is
     * guaranteed only when every scanner in {@link #scanners} return smaller or equal row key when
     * {@link HaeinsaKeyValueScanner#peek()} is called.
     *
    ...
    public HaeinsaResult next() throws IOException {
      ...
      while (true) {
        ...
        if (prevKV == null) {
          ...
          if (lockInclusive) {
            // HaeinsaKeyValues from HBaseScanScanner or HBaseGetScanner contains TRowLock for this
            // row.
            ...
            if (rowState.getCurrent() == null) {
              ...
              // rowState is already exist, use current variable in rowState instead of TRowLock
              // from scan
              // or get
            }
        ...
        if (Bytes.equals(prevKV.getRow(), currentKV.getRow())) {
          ...
          } else if (currentKV.getType() == Type.DeleteColumn
              || currentKV.getType() == Type.DeleteFamily) {
          ...
          if (sortedKVs.size() > 0) {
            // If currentKV moved to next row and there are more than one KV satisfy scan
            // requirement,
    ...
    /**
     * Moving index of scanner of currentScanner by one. If there is no more element at the scanner,
     * remove currentScanner from scanners (NavigableSet)
     */
   ...
   * This class is used if there are {@link HaeinsaScan} or {@link HaeinsaIntraScan} inside
   * transaction.
   *
   * <p>HBaseScanScanner can contain kev-values for single row like {@link ResultScanner}, or
   * key-values of multiple rows.
   *
   * <p>HBaseScanScanner does not need sequenceID control because it will use {@link
   * HBaseGetScanner} if rows from scan do not have stable state, and put result from get to lower
   * sequenceID. Therefore, {@link #getSequenceID()} always return Long.MAX_VALUE
   */
  private static class HBaseScanScanner implements HaeinsaKeyValueScanner {
    ...
    /**
     * currentResult is null when there is no more elements to scan in resultScanner or scan is not
     * started. currentResult only contains result of single row.
     */
    ...
    public HaeinsaKeyValue peek() {
      try {
        ...
        // resultIndex.
        current =
            new HaeinsaKeyValue(KeyValueUtil.ensureKeyValue(currentResult.rawCells()[resultIndex]));
        resultIndex++;
  ...
  /**
   * Implementation of {@link HaeinsaKeyValueScanner} which provides scanner interface which wrap
   * {@link KeyValue}s from {@link Result} by HBase {@link Get} to {@link HaeinsaKeyValue}s. This
   * class is used if there is {@link HaeinsaGet} inside transaction.
   *
   * <p>HBaseGetScanner can contain key-values for only single row link {@link Result}. So {@link
   * #peek()} method or {@link #next()} method will return value from same row.
   *
   * <p>HBaseGetScanner is now used in two cases. First, inside {@link
   * HaeinsaTable#get(HaeinsaTransaction, HaeinsaGet)}, {@link HaeinsaKeyValue}s returned by this
   * class will always have sequenceId of Long.MAX_VALUE. Second is used inside {@link
   * ClientScanner}. In this case, HBaseGetScanner recover failed transaction and get recovered data
   * from HBase. Recovered {@link HaeinsaKeyValue} should have smaller sequenceId contained in
   * ClientScanner so far because it is more recent value.
   */
  private static class HBaseGetScanner implements HaeinsaKeyValueScanner {
    ...
    public HaeinsaKeyValue peek() {
      ...
      current =
          new HaeinsaKeyValue(KeyValueUtil.ensureKeyValue(result.listCells().get(resultIndex)));
      resultIndex++;
      return current;
    }
