...
/**
 * Modified POJO container of {@link Result} class in HBase. Link {@link Result} , can contain
 * multiple {@link HaeinsaKeyValue}. All HaeinsaKeyValue in HaeinsaResult are from same row.
 */
public class HaeinsaResult {
  ...
  public HaeinsaResult(Result result) {
    if (result.isEmpty()) {
      ...
      List<HaeinsaKeyValue> transformed =
          Lists.transform(
              result.listCells(),
   ...
   *
   * @param sortedKVs - Assumed that {@link HaeinsaKeyValue}s in sortedKVs have same row with first
  ...
  public byte[] getValue(byte[] family, byte[] qualifier) {
    ...
    int pos =
        Collections.binarySearch(
            sortedKVs,
            new HaeinsaKeyValue(row, family, qualifier, null, KeyValue.Type.Maximum),
