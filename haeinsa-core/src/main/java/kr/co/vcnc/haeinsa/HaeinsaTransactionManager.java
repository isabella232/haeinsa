...
/**
 * Manager class of {@link HaeinsaTransaction}. This class contains {@link HaeinsaTablePool} inside
 * to provide tablePool when user want to access HBase through {@link HaeinsaTransaction} with
 * {@link HaeinsaTable} and execute transaction.
 *
 * <p>HaeinsaTransactionManager also provides method to recover failed transaction from TRowLock in
 * HBase which can be used to clear it up or complete it.
 */
public class HaeinsaTransactionManager {
  ...
  /**
   * Make new {@link HaeinsaTransaction} instance which can be used to recover other
   * failed/uncompleted transaction. Also read and recover primaryRowKey and primaryRowLock from
   * failed transaction on HBase.
   *
   * <p>This method is thread-safe.
   *
  ...
  /**
   * Get Unstable state of {@link TRowLock} from given row. Returns null if {@link TRowLock} is
   * {@link TRowLockState#STABLE}.
   *
  ...
  /**
   * Check if given {@link TRowLock} is dangling RowLock. RowLock is in dangling if the RowLock is
   * secondary lock and the primary of the RowLock doesn't have the RowLock as secondary.
   *
   ...
   * @throws IOException When error occurs. Especially throw {@link DanglingRowLockException}if
   *     given RowLock is dangling.
   */
  private void checkDanglingRowLockOrThrow(byte[] tableName, byte[] row, TRowLock rowLock)
      throws IOException {
    ...
    if (Objects.equal(previousRowLock, currentRowLock)) {
      if (!TRowLocks.isPrimary(currentRowLock)) {
        ...
        if (!TRowLocks.isSecondaryOf(
            primaryRowKey, primaryRowLock, secondaryRowKey, currentRowLock)) {
          throw new DanglingRowLockException(
              secondaryRowKey, "Primary lock doesn't have rowLock as secondary.");
        }
      }
    }
  }
  ...
  /**
   * Recover TRowLocks of failed HaeinsaTransaction from primary row on HBase. Transaction
   * information about secondary rows are recovered with {@link
   * #addSecondaryRowLock(HaeinsaTransaction, TRowKey, TRowLock, TRowKey)}. HaeinsaTransaction made
   * by this method do not assign proper values on mutations variable.
   */
  private HaeinsaTransaction getTransactionFromPrimary(TRowKey rowKey, TRowLock primaryRowLock)
      throws IOException {
    ...
    HaeinsaTableTransaction primaryTableTxState =
        transaction.createOrGetTableState(rowKey.getTableName());
    HaeinsaRowTransaction primaryRowTxState =
        primaryTableTxState.createOrGetRowState(rowKey.getRow());
  ...
  /**
   * Recover TRowLock of secondary row inferred from {@link TRowLock#secondaries} field of primary
   * row lock.
   *
   * <p>If target secondary row is in stable state, the row does not included in recovered
   * HaeinsaTransaction because it suggest that this secondary row is already stabled by previous
   * failed transaction.
   *
   * <p>Secondary row is not included in recovered transaction neither when commitTimestamp is
   * different with primary row's, because it implicates that the row is locked by other
   * transaction.
   *
   * <p>As similar to {@link #getTransactionFromPrimary(TRowKey, TRowLock)}, rowTransaction added by
   * this method do not have proper mutations variable.
   */
  private void addSecondaryRowLock(
      HaeinsaTransaction transaction,
      TRowKey primaryRowKey,
      TRowLock primaryRowLock,
      TRowKey secondaryRowKey)
      throws IOException {
    TRowLock secondaryRowLock =
        getRowLock(secondaryRowKey.getTableName(), secondaryRowKey.getRow());
    ...
    if (secondaryRowLock.getState() == TRowLockState.STABLE
        && secondaryRowLock.getCommitTimestamp() == transaction.getCommitTimestamp()) {
      // this row is already committed or aborted.
      return;
    }
    if (secondaryRowLock.getState() != TRowLockState.STABLE
        && !TRowLocks.isSecondaryOf(
            primaryRowKey, primaryRowLock, secondaryRowKey, secondaryRowLock)) {
      // this row isn't a part of this transaction.
      return;
    }
    HaeinsaTableTransaction tableState =
        transaction.createOrGetTableState(secondaryRowKey.getTableName());
