...
/**
 * Implementation of {@link HaeinsaMutation} which only contains HaeinsaKeyValue with {@link
 * Type#Put} identifier. HaeinsaPut can be analogous to {@link Put} class in HBase.
 *
...
public class HaeinsaPut extends HaeinsaMutation {
  ...
  public HaeinsaPut(HaeinsaPut putToCopy) {
    ...
    for (Map.Entry<byte[], NavigableSet<HaeinsaKeyValue>> entry :
  ...
  /**
   * Creates an empty set if one doesn't exist for the given column family or else it returns the
   * associated set of KeyValue objects.
   *
   * @param family column family
   * @return a set of KeyValue objects, returns an empty set if one doesn't exist.
   */
