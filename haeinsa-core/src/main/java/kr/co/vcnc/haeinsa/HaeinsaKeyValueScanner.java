...
/**
 * Scanner wrapper of HaeinsaKeyValue. Contains multiple HaeinsaKeyValue inside to allow iterator
 * pattern.
 *
...
public interface HaeinsaKeyValueScanner {
  Comparator<HaeinsaKeyValueScanner> COMPARATOR =
      new Comparator<HaeinsaKeyValueScanner>() {
        @Override
  ...
  Comparator<HaeinsaKeyValueScanner> REVERSE_COMPARATOR =
      new Comparator<HaeinsaKeyValueScanner>() {
        @Override
        public int compare(HaeinsaKeyValueScanner o1, HaeinsaKeyValueScanner o2) {
          return ComparisonChain.start()
  ...
  /**
   * Get the sequence id associated with this KeyValueScanner. This is required for comparing
   * multiple KeyValueScanners to find out which one has the latest data. The default implementation
   * for this would be to return 0. A KeyValueScanner having lower sequence id will be considered to
   * be the newer one.
   */
  long getSequenceID();
