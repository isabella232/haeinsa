...
/**
 * Exception when transaction meet dangling RowLock during execution. RowLock is in dangling if the
 * RowLock is secondary lock and the primary of the RowLock doesn't have the RowLock as secondary.
 * Dangling row can be retrieve with {@link #getDanglingRowKey}
 *
 * <p>Dangling rowLock should not be appear normal case, but we throw this exception if there is
 * fatal consistency error in Haeinsa.
 */
public class DanglingRowLockException extends ConflictException {
  ...
  /**
   * This method returns {@link TRowKey} of Dangling row. You can get dangling rowLock with this row
   * key.
   *
