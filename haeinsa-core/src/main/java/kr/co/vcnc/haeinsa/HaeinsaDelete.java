 ...
 * Type#DeleteFamily} and {@link Type#DeleteColumn} identifier. HaeinsaPut can be analogous to
 * {@link Delete} class in HBase.
 *
...
public class HaeinsaDelete extends HaeinsaMutation {
   ...
   *
   * <p>Overrides previous calls to deleteColumn and deleteColumns for the specified family.
   *
  ...
  public TMutation toTMutation() {
    ...
    for (HaeinsaKeyValue kv : Iterables.concat(familyMap.values())) {
      switch (kv.getType()) {
        case DeleteColumn:
          {
            newTRemove.addToRemoveCells(
                new TCellKey().setFamily(kv.getFamily()).setQualifier(kv.getQualifier()));
            break;
          }
        case DeleteFamily:
          {
